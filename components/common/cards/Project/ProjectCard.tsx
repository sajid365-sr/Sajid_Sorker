...
//
import { motionStep } from "shared/config/motion";
  ...
  return (
    ...
    >
      ...
      <div>
          ...
          <div className={"flex justify-end items-center gap-3 text-slate-300"}>
            {live && (
              <Link
                href={`${live}`}
                target="_blank"
                className="text-sky-400 transition-all duration-300 hover:scale-110 text-2xl"
              >
            ...
            {code && (
              <Link
                href={`${code}`}
                target="_blank"
                className="text-sky-400  transition-all duration-300 hover:scale-110 text-xl"
              >
                <FiGithub />
              </Link>
            )}
        ...
        <div className="flex  text-slate-300 gap-3 mt-7 gap-x-4 flex-wrap">
          {tech.map((e: string, i: number) => (
            <span className="bg-[#0F172A]/50 px-3 rounded-full" key={i}>
              {e}
            </span>
          ))}
        </div>
